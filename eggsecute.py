...
# modified version taken from https://github.com/blacklocus/aurora-echo
...
def main(script_path, output_path):
    ...
    module_files.extend(collect_module_files('pykeepass', ''))
    module_files.extend(collect_module_files('Crypto', ''))
    module_files.extend(collect_module_files('construct', ''))
    # module_files.extend(collect_single_module_file('construct'))

    # filter out everything but .py's
    filtered_files = [x for x in module_files if x[1].endswith(".py") or x[1].endswith(".json") or x[1].endswith(".pem") or x[1].endswith(".so")]
